...
async function fetchData() {
  ...
  try {
    ...
    console.error(e);
    if(listDiv) listDiv.innerHTML = "<p style='color:red;'>データ取得に失敗しました。再読み込みしてください。</p>";
  }
}
...
function openModal(id, mode) {
  ...
  if (mode === 'view') {
    ...
    if (p.status !== "未入金") {
      ...
      qrHtml += `<div style="font-size:0.75rem; font-weight:bold; color:#b45309; margin-bottom:10px; text-align:center;">🎫 入場用QRコード（人数分：${totalCount}個）</div>`;
      ...
      for (let i = 1; i <= totalCount; i++) {
        ...
        qrHtml += `
          <div style="text-align:center; background:#fff; padding:5px; border:1px solid #eee; border-radius:5px; width:100px;">
            ...
            <div style="font-size:0.55rem; color:#666; font-family:monospace;">${branchId}</div>
          </div>`;
      }
      qrHtml += `</div></div>`;
    } else {
...
function printTicket(id) {
  ...
  // 人数分だけ独立した枠（A4縦に並ぶサイズ）を作成
  for (let i = 1; i <= totalTickets; i++) {
    const individualId = `${p.id}-${i}`;
    const ticketDiv = document.createElement("div");
    ticketDiv.style.cssText = "width:185mm; height:68mm; border:1px solid #333; margin:2mm auto; display:flex; font-family:sans-serif; background:#fff; page-break-inside:avoid; position:relative;";
    ...
    ticketDiv.innerHTML = `
      <div style="flex:3; padding:15px; border-right:1.5mm dashed #000; position:relative; text-align:left;">
        ...
        <div>
          <p style="font-size:0.65rem; margin:0;">5th Anniversary solo performance "Mabui"</p>
