  ...
  data.forEach(row => {
    ...
    item.innerHTML = `
      <div>
        ...
        <div class="order-name">${row.name} 様</div>
      </div>
...
function openModal(id, mode) {
  ...
  if (mode === 'view') {
    ...
    const isPaid = (p.status === "入金済み");

    body.innerHTML = `
      <div style="background:#f8fafc; padding:15px; border-radius:10px; font-size:0.85rem; border:1px solid #e2e8f0; line-height:1.6;">
        <div style="font-weight:bold;">${p.name} 様 (${p.id})</div>
        <div>〒${p.zip||''} ${p.pref||''}${p.city||''}${p.rest||''}</div>
        <div>📞 ${p.tel} / ✉️ ${p.email}</div>
        ...
        <div style="font-size:0.75rem; color:#64748b; margin-top:5px; background:#fff; padding:5px; border-radius:4px;">備考: ${p.remarks || 'なし'}</div>
      </div>
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:15px;">
        <button onclick="updateStatus('${p.id}', '入金済み')" ${(isQR && isPaid) ? 'disabled style="background:#cbd5e0; color:#94a3b8;"' : 'style="background:#10b981; color:white;"'} class="action-btn">💰 入金確認</button>
        ...
        <button onclick="deleteOrder('${p.id}')" style="background:#ef4444; color:white;" class="action-btn">🗑 削除</button>
      </div>
    `;
  } else {
    body.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:8px; max-height:60vh; overflow-y:auto; padding:5px;">
        <div style="background:#f1f5f9; padding:10px; font-weight:bold; border-radius:8px;">ID: ${p.id} / 名前: ${p.name} (変更不可)</div>
        <input type="text" id="edit-zip" value="${p.zip||''}" placeholder="郵便番号" onblur="autoZip(this.value)" class="edit-input">
        <input type="text" id="edit-pref" value="${p.pref||''}" placeholder="都道府県" class="edit-input">
        <input type="text" id="edit-city" value="${p.city||''}" placeholder="市区町村" class="edit-input">
        <input type="text" id="edit-rest" value="${p.rest||''}" placeholder="番地・建物" class="edit-input">
        <input type="tel" id="edit-tel" value="${p.tel||''}" placeholder="電話番号" class="edit-input">
        <input type="email" id="edit-email" value="${p.email||''}" placeholder="メール" class="edit-input">
        <div class="edit-grid">
          <input type="number" id="edit-sa" value="${p.s_a}" oninput="reCalc()" placeholder="S大" class="edit-input">
          <input type="number" id="edit-sc" value="${p.s_c}" oninput="reCalc()" placeholder="S子" class="edit-input">
          <input type="number" id="edit-ga" value="${p.g_a}" oninput="reCalc()" placeholder="般大" class="edit-input">
          <input type="number" id="edit-gc" value="${p.g_c}" oninput="reCalc()" placeholder="般子" class="edit-input">
        </div>
        ...
        <input type="number" id="edit-total" value="${p.total}" style="font-weight:bold; color:red;" class="edit-input">
        <textarea id="edit-remarks" placeholder="備考" class="edit-input">${p.remarks||''}</textarea>
        <button onclick="saveEdit()" style="background:#1e3a8a; color:white; padding:15px; border-radius:8px; font-weight:bold; border:none; margin-top:5px; cursor:pointer;">💾 この内容で上書き保存</button>
      </div>`;
  }
...
function reCalc() {
  ...
  document.getElementById("edit-total").value = (sa+sc)*3500 + (ga+gc)*1500;
}
...
async function deleteOrder(id) { if(confirm("キャンセル（削除）しますか？")) { await fetch(url, { method: "POST", body: JSON.stringify({ type: "updateStatus", id: id, status: "キャンセル" }) }); fetchData(); closeModal(); } }
